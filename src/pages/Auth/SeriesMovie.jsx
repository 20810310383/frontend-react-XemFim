...
const SeriesMovie = () => {
  ...
  useEffect(() => {
    const fetchSeriesMovies = async () => {
      ...
      try {
        setLoading(true);
        const response = await axios.get(`${config.API_URL}/movies/series`);
  ...
  const handleMovieClick = (movie) => {
    navigate(`/detail/${movie.slug}`); //Điều hướng đến chi tiết phim khi chọn phim
  };
  ...
  return (
    ...
    <div className="container mx-auto px-4 bg-black text-white">
      <NavBar />
      <h1 className="text-3xl font-bold my-6">Phim Bộ</h1>
